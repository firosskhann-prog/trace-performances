...
// Recognition tuning: faster scan + more features.
const MAX_SCREEN_POINTS = 700;
const CORNER_POOL = 10000;
const MAX_PATTERN_SIZE = 512;
const MAX_PATTERN_POINTS = 420;
const TRAIN_LEVELS = 5;
...
let waitingForTraceClear = false;
let clearFrames = 0;
let trained = false;
...
// One strong frame is enough to trigger.
...
function initVision() {
  ...
  imgU8 = new jsfeat.matrix_t(
    W,
    H,
    jsfeat.U8_t | jsfeat.C1_t
  );

  imgSmooth = new jsfeat.matrix_t(
    W,
    H,
    jsfeat.U8_t | jsfeat.C1_t
  );
  ...
  screenCorners = Array.from(
    { length: CORNER_POOL },
    () => new jsfeat.keypoint_t(0, 0, 0, 0, -1)
  );

  matches = Array.from(
    { length: MAX_SCREEN_POINTS },
    () => ({
      screen_idx: 0,
      pattern_lev: 0,
      pattern_idx: 0,
  ...
  homo3x3 = new jsfeat.matrix_t(
    3,
    3,
    jsfeat.F32C1_t
  );

  matchMask = new jsfeat.matrix_t(
    MAX_SCREEN_POINTS,
    1,
    jsfeat.U8C1_t
  );
...
function detectKeypoints(img, corners, maxAllowed) {
  ...
  yape06.laplacian_threshold = 22;
  yape06.min_eigen_value_threshold = 18;
...
function trainPatternFromCanvas() {
  ...
  const sc0 = Math.min(
    1,
    MAX_PATTERN_SIZE / sourceW,
    MAX_PATTERN_SIZE / sourceH
  );
  ...
  const sourceData = patternCtx.getImageData(
    0,
    0,
    sourceW,
    sourceH
  ...
  const sourceGray = new jsfeat.matrix_t(
    sourceW,
    sourceH,
    jsfeat.U8_t | jsfeat.C1_t
  );

  const level0 = new jsfeat.matrix_t(
    baseW,
    baseH,
    jsfeat.U8_t | jsfeat.C1_t
  );

  const levelImg = new jsfeat.matrix_t(
    baseW,
    baseH,
    jsfeat.U8_t | jsfeat.C1_t
  );
  ...
  trained = totalPoints >= 100;

  return totalPoints;
}

traceInput.addEventListener(
  'change',
  async () => {
    ...
    if (!jsfeat) {
      setStatus(
        'Recognition engine is still loading.',
        'Try the trace again in a moment.'
      );
    ...
    img.onload = () => {
      ...
      const scale = Math.min(
        1,
        maxInput / img.naturalWidth,
        maxInput / img.naturalHeight
      );
      ...
      try {
        const points =
          trainPatternFromCanvas();

        traceReady = trained;

        traceLabel.classList.toggle(
          'picked',
          traceReady
        );

        traceLabel.firstChild.textContent =
          traceReady
            ? 'TRACE READY ✓ '
            : 'CHOOSE A DIFFERENT TRACE ';
        ...
        if (traceReady) {
          ...
          setStatus(
            'This trace is too plain.',
            'Choose an image with more edges, texture, lettering or detail.'
          );
        }
      } catch (err) {
        console.error(err);
...
soundInput.addEventListener(
  ...
  async () => {
    ...
    try {
      ...
      soundLabel.classList.add(
        'picked'
      );

      soundLabel.firstChild.textContent =
        'SOUND READY ✓ ';

      setStatus(
        traceReady
          ? 'Trace and sound are ready.'
          : 'Sound prepared. Now choose the trace image.'
      );
    } catch (err) {
      console.error(err);
      ...
      setStatus(
        'Could not read that sound file.',
...
function resetDetectionForNextTrace() {
  resetGate();
  ...
  waitingForTraceClear = true;
  clearFrames = 0;

  setStatus(
    'SOUND ENDED — READYING NEXT TRACE…',
    'Move away from this trace, then show the next one.'
  );
}
...
function playSound() {
  ...
  setStatus(
    'TRACE FOUND — SOUND PLAYING',
    'Recognition is paused until this sound ends.'
  );
}
...
async function startCamera() {
  ...
  try {
    ...
    setStatus(
      'SCANNING…',
      'Point the camera at the selected trace.'
    );
...
function processFrame() {
  ...
  try {
    ...
    // After sound ends, the old trace
    // must disappear before another trigger.
    if (
      waitingForTraceClear
    ) {
      cameraBox.classList.remove(
        'found'
      );
      ...
      if (
        ...
        setStatus(
          'READYING NEXT TRACE…',
          'Move the camera away from the previous trace.'
        );
      }
    ...
    if (state.found) {
      setStatus(
        'TRACE FOUND',
        `${goodMatches} geometric matches / ${numMatches} candidate matches`
      );
    } else {
      setStatus(
        'SCANNING…',
    ...
    setStatus(
      'Recognition paused because of an error.',
